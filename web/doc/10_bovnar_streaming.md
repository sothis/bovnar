...
- A message is **length-prefixed**, so it may span any number of chunks, and
  ...
  reassembles each channel independently.
- A message that fits `BVNR_MUX_MAX_MESSAGE` travels in a single chunk (fast path).
- **The one producer constraint:** a message's length varint is never split
  across two chunks. `bvnr_mux_send` guarantees this (the fragment capacity is
  ~64 KiB and the length varint is ≤ 10 bytes, so it always rides the first
...
latches `bvnr_demux_error` and aborts the read.
