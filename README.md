...
- **Extensively tested** — Unit tests, socket-pair round-trip tests, a 347-case conformance suite, fuzz harnesses (reader, writer, DOM, utils), and a built-in benchmark mode (`bovnar bench`).
...
| `bvnr_conformance` | 347-case conformance suite — self-test plus `--iut` adapter mode |
...
| [Conformance Test Tool](doc/7_bovnar_conformance.md) | Conformance suite (347 cases), IUT protocol for verifying third-party implementations, TAP output, and CTest integration. |
